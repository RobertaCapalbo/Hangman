...
export default function Jogo({wordGuess, guessedLetters, numberOfGuesses, reveal, setWordGuess, words, imagem, setImagem}) {
  ...
  function chooseWord(){
    ...
    <div className="container-imagens">
      ...
      {wordGuess.split('').map((letter,index) => (<span style={{borderBottom:'3px solid black', width: '50px'}} key={index}><span style={{visibility:guessedLetters.includes(letter) || reveal?'visible':'hidden', visibility: guessedLetters.includes(letter) || reveal ? letter : '_', color: !guessedLetters.includes(letter) && reveal ? 'red' : 'green'}}>{letter}</span></span>))}
      </div>
      </div>
      </div>
    );
